class AddRpushApps < ActiveRecord::Migration
	def self.up
		...
		Rpush::Apns::App.create!(name: 'Activer_Apns', certificate: File.read('/etc/ssl/activer_apns_sandbox.pem') + File.read('/etc/ssl/activer_apns_sandbox.key'), environment: 'sandbox', password: 'password', connections: 1) if Rails.env.production?
